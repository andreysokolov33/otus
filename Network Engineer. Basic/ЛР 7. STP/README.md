...
3. Теперь идет сравнение стоимости порта соседа. **S1** получает BPDU на порты ```Fa0/1``` и ```Fa0/2```, в которых указан порт **S3**. BPDU, пришедшие с порта ```Fa0/1```, считаются приоритетными, поэтому порт ```Fa0/1``` **S2** переходит в состояние **Root порта**, а ```Fa0/2``` в состояние **Alternate**. Аналогичная логика для **S2**, только ему BPDU приходят с портов ```Fa0/3``` и ```Fa0/4``` **S3**. Цена ```Fa0/3``` меньше, чем у ```Fa0/4```, поэтому ```Fa0/1``` **S2**, который непосредственно подключен к ```Fa0/3``` **S3**, будет в состоянии **Root порта**, а ```Fa0/2``` перейдет в **Alternate**. 
...
**Ответ**. Если и **Bridge Id** одинаковый, сравниваться будут уже **стоимости портов** соседа. Чем ниже, тем приоритетнее.    