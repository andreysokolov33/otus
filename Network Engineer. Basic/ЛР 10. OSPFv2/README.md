...
**Ответ**. Маршрутизатора R1 стал DR, а R2 стал BDR. Выбор осущестлялся по RouterID, так как приоритет на данный момент у них одинаковый. У R1 RouterID равен 1.1.1.1, а у R2 2.2.2.2
...
Результат выполнения команды `show ip route ospf` на R2. В выводе присутствует лишь маршрут по умолчанию

![Alt text](./r2-show-ip-route-ospf.png)

Пинг с R2 до Loopback интерфейса R1 прошел успешно

![Alt text](./ping-from-r2-to-lo-r1.png)


**Вопрос**. Почему стоимость OSPF для маршрута по умолчанию отличается от стоимости OSPF в R1 для сети 192.168.1.0/24?

**Ответ**. Вероятно, дефолтный маршрути имеет метрику 1, чтобы он был всегда доступен, чтобы при изменении цены на каком-то интерфейсе не приходилось прерывать передачу трафика на гейтвей, чтобы высчитать более оптимальный маршрут