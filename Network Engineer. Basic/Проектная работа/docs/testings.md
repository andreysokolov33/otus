...
За счет опции preempt, если Active интерфейс недоступен в схеме, то Standby интерфейс перейдет в статус Active. Например, на CORE2-SW, который является Active для VLAN 77, отключили данный интерфейс. Через несколько секунд на CORE1-SW появилась информация, что теперь интерфейс VLAN 77 является Active:
...
Результат команды `show etherchannel summary` на CORE1-SW:

![Alt text](../images/2.1-etherchannel-core1.png)

CORE2-SW

![Alt text](../images/2.1-etherchannel-core2.png)

Агрегированный канал работает
...
--- 
Для тестирования отказоустойчивости был пущен пинг с флагом -t из сети VLAN 80. Затем отключены все порты на CORE2-SW, через который должен проходить трафик из данной сети. На картинке видно, что было потеряно 7 пакетов до того, как всю работу на себя взял CORE1-SW. Затем порты на CORE2-SW включены снова, что повлекло еще к 6 потерянным пакетам. Далее трафик начал идти так, как и должен
